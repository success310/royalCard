...
class AuthController extends Controller
{
    ...
    public function postLogin(LoginRequest $request, AuthService $authService): RedirectResponse
    {
        ...
        if ($success) {
            // If successful, redirect to the admin index page
            return redirect()->intended(route('admin.index'));
        } else {
            // If not successful, redirect back to the login page with an error message and preserve input (except password)
